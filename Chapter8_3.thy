...
"ccomp (REPEAT c UNTIL b) =
 (let cc = ccomp c; cb = bcomp b True 1 in cc @ cb @ [JMP (-(size cb + size cc + 1))])"
...
next
  case (RepeatTrue b s2 c s1 s3 stk)
   (* ccomp (REPEAT c_ UNTIL b_) \<turnstile> (0, s\<^sub>1_, stk) \<rightarrow>* (size (ccomp (REPEAT c_ UNTIL b_)), s\<^sub>3_, stk) *)
   let ?cc = "ccomp c"
   let ?cb = "bcomp b True 1"
   let ?prog = "ccomp (REPEAT c UNTIL b)"
   have prog_def: "?prog = ?cc @ ?cb @ [JMP (-(size ?cc + size ?cb + 1))]" by auto
   from RepeatTrue.IH have "?prog \<turnstile> (0, s1, stk) \<rightarrow>* (size ?cc, s2, stk)" by fastforce
   moreover from `\<not>bval b s2` bcomp_correct[where n=1 and b=b and f=True and s=s2 and stk=stk]
     have "?cb \<turnstile> (0, s2, stk) \<rightarrow>* (size ?cb, s2, stk)"
     by auto
   moreover have "?cc @ ?cb \<turnstile> (size ?cc, s2, stk) \<rightarrow>* (size ?cc + size ?cb, s2, stk)" using exec_appendL calculation by auto
   moreover have "?prog \<turnstile> (size ?cc, s2, stk) \<rightarrow>* (size ?cc + size ?cb, s2, stk)" using exec_appendR calculation prog_def append_assoc by metis
   moreover have "?prog \<turnstile> (size ?cc + size ?cb, s2, stk) \<rightarrow> (0, s2, stk)" by auto
   moreover have "?prog \<turnstile> (0, s1, stk) \<rightarrow>* (0, s2, stk)" by (meson star_trans calculation prog_def star_step1)
   ultimately show ?case using RepeatTrue.IH star_trans by smt
   (* FIXME smt: just needs suitable instantiation of star_trans *)
qed fastforce+
