...
Or1:     "(a OR b, s) \<rightarrow> (a, s)" |
Or2:     "(a OR b, s) \<rightarrow> (b, s)"
...
inductive_cases OrE[elim!]: "(a OR b, s) \<rightarrow> ct"
...
next
  case (Or1 a s1 s2 b)
  have "(a OR b, s1) \<rightarrow>* (a, s1)" by auto
  moreover have "(a, s1) \<rightarrow>* (SKIP, s2)" using Or1.IH by auto
  ultimately show ?case by (metis star_trans)
next
  case (Or2 b s1 s2 a)
  have "(a OR b, s1) \<rightarrow>* (b, s1)" by auto
  moreover have "(b, s1) \<rightarrow>* (SKIP, s2)" using Or2.IH by auto
  ultimately show ?case by (metis star_trans)
qed
...
next
  case Or1
  thus ?case by (metis star.step small_step.Or1)
next
  case Or2
  thus ?case by (metis star.step small_step.Or2)
qed
