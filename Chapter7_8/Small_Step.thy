...
next
  case (RepeatFalse b s2 c s1)
  let ?rep = "REPEAT c UNTIL b"
  let ?exp = "IF b THEN SKIP ELSE ?rep"
  have "(?rep, s1) \<rightarrow>* (c;; ?exp, s1)" by auto
  moreover have "(c;; ?exp, s1) \<rightarrow>* (SKIP;; ?exp, s2)" using RepeatFalse.IH RepeatFalse.hyps star_seq2 by auto
  moreover have "(SKIP;; ?exp, s2) \<rightarrow>* (?exp, s2)" by auto
  moreover have "(?exp, s2) \<rightarrow>* (SKIP, s2)" using `bval b s2` by auto
  ultimately show ?case by (metis star_trans)
next
  case (RepeatTrue b s2 c s1 s3)
  let ?rep = "REPEAT c UNTIL b"
  let ?exp = "IF b THEN SKIP ELSE ?rep"
  have rep_big: "(?rep, s1) \<Rightarrow> s3" using RepeatTrue.hyps by blast
  have "(?rep, s1) \<rightarrow>* (c;; ?exp, s1)" by auto
  moreover have "(c;; ?exp, s1) \<rightarrow>* (SKIP;; ?exp, s2)" using `(c, s1) \<rightarrow>* (SKIP, s2)` star_seq2 by auto
  moreover have "(SKIP;; ?exp, s2) \<rightarrow>* (?exp, s2)" by auto
  moreover have "(?exp, s2) \<rightarrow>* (?rep, s2)" using `\<not>bval b s2` by auto
  moreover have "(?rep, s2) \<rightarrow>* (SKIP, s3)" using `(?rep, s2) \<rightarrow>* (SKIP, s3)` by auto
  ultimately show ?case by (metis star_trans)
qed
...
next
  case RepeatFalse
  thus ?case by (simp add: RepeatFalse.hyps(1) RepeatFalse.hyps(2) big_step.RepeatFalse big_to_small)
next
  case RepeatTrue
  thus ?case using big_step.RepeatTrue big_to_small by auto
qed
